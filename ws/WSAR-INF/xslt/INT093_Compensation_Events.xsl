...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="wd:Report_Data">
        <xsl:element name="{local-name()}">
            <xsl:attribute name="Report">
                <xsl:text>Compensation_Events</xsl:text>
            </xsl:attribute>
            <xsl:apply-templates select="node() | @*"/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="wd:Report_Entry">
        <Report_Entry>
            ...
            <!-- <EMPLID><xsl:value-of select="wd:Worker_group/wd:SEMA4_ID"/></EMPLID> -->
            <POSITID><xsl:value-of select="wd:Position/wd:Position_ID"/></POSITID>
            <EFFECTIVEDATE><xsl:value-of select="wd:Effective_Date"/></EFFECTIVEDATE>
            <BIWEEKLYAMOUNT><xsl:value-of select="format-number(wd:BW_Hourly_Rate,'#.00')"/></BIWEEKLYAMOUNT>
            <Job_Record_Number><xsl:value-of select="wd:Position/wd:Job_Record_Number/wd:ID[@wd:type='Custom_Organization_Reference_ID']"/></Job_Record_Number>
            <ChargeInsuranceRetirement><xsl:value-of select="wd:Position/wd:Position_Charge_Insurance"/></ChargeInsuranceRetirement>
