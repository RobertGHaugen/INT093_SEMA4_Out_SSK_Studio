<xsl:stylesheet 
    ...
    <xsl:template match="wd:Import_Payroll_Input_Request">
        <xsl:param name="lookup" as="item()?"/>
        ...
        <wd:Messages>
            ...
            <xsl:iterate select="*">
                ...
                <xsl:param name="batchRecord" as="item()?"/>
                ...
                <xsl:choose>
                    <xsl:when test="exists($p)">
                        ...
                        <xsl:if test="exists($messageRecord)">
                            <xsl:apply-templates select=".">
                                ...
                                <xsl:with-param name="batchRecord" select="$batchRecord"/>
                    ...
                    <xsl:otherwise>
                        <xsl:variable name="br" as="item()?" select="snapshot()"/>
                        <xsl:apply-templates select="key('lookupKey', '0', $lookup)" mode="in-memory">
                            <xsl:with-param name="batchRecord" select="$br"/>
                        </xsl:apply-templates>
                        <xsl:next-iteration>
    ...
    <xsl:template match="wd:Payroll_Input_Data">
        <xsl:param name="p" as="xsd:integer"/>
        <xsl:param name="batchRecord" as="item()"/>
    ...
    <xsl:template match="wd:Import_Process_Message" mode="in-memory">
        ...
        <xsl:message select="cl:createMessage(
            ...
            normalize-space(serialize($batchRecord)),
