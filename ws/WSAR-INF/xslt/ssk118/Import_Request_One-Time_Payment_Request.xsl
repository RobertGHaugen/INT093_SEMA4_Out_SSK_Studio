<xsl:stylesheet 
    ...
    <xsl:template match="wd:Import_Request_One-Time_Payment_Request">
        <xsl:param name="lookup" as="item()?"/>
        ...
        <wd:Messages>
            ...
            <xsl:iterate select="*">
                ...
                <xsl:param name="idRecord" as="item()?"/>
                ...
                <xsl:choose>
                    <xsl:when test="exists($p)">
                        ...
                        <xsl:if test="exists($messageRecord)">
                            <xsl:apply-templates select=".">
                                ...
                                <xsl:with-param name="idRecord" select="$idRecord"/>
                    ...
                    <xsl:otherwise>
                        <xsl:variable name="ir" as="item()?" select="snapshot()"/>
                        <xsl:apply-templates select="key('lookupKey', '0', $lookup)" mode="in-memory">
                            <xsl:with-param name="idRecord" select="$ir"/>
                        </xsl:apply-templates>
                        <xsl:next-iteration>
    ...
    <xsl:template match="wd:Request_One-Time_Payment_Request_Information_HV">
        <xsl:param name="p" as="xsd:integer"/>
        <xsl:param name="idRecord" as="item()"/>
    ...
    <xsl:template match="wd:Import_Process_Message" mode="in-memory">
        ...
        <xsl:message select="cl:createMessage(
            ...
            normalize-space(serialize($idRecord)),
