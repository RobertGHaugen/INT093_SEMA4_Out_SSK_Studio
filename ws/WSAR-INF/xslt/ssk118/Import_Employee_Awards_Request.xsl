<xsl:stylesheet 
    ...
    <xsl:template match="wd:Import_Employee_Awards_Request">
        <xsl:param name="lookup" as="item()?"/>
        ...
        <wd:Messages>
            ...
            <xsl:iterate select="*">
                ...
                <xsl:param name="eventRecord" as="item()?"/>
                ...
                <xsl:choose>
                    <xsl:when test="exists($p)">
                        ...
                        <xsl:if test="exists($messageRecord)">
                            <xsl:apply-templates select=".">
                                ...
                                <xsl:with-param name="eventRecord" select="$eventRecord"/>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:apply-templates select="key('lookupKey', '0', $lookup)" mode="in-memory">
                            <xsl:with-param name="eventRecord" select="$ir"/>
                        </xsl:apply-templates>
                        <xsl:next-iteration>
    ...
    <xsl:template match="wd:Employee_Award_Data">
        <xsl:param name="p" as="xsd:integer"/>
        <xsl:param name="eventRecord" as="item()"/>
    ...
    <xsl:template match="wd:Import_Process_Message" mode="in-memory">
        ...
        <xsl:message select="cl:createMessage(
            ...
            normalize-space(serialize($eventRecord)),
