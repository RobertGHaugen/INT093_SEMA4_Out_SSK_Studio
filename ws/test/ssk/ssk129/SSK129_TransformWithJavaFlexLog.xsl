...
<xsl:stylesheet 
    ...
    exclude-result-prefixes="xs tube ctx ssk err"
    version="3.0">
    
    <xsl:param name="sskXsltLibFlexLogExtensions" as="xs:string" select="ctx:getProperty(tube:getCurrentMediationContext(),'sskXsltLibFlexLogExtensions')" static="yes"/>
	<!--
	...
	-->
    <!-- <xsl:param name="sskXsltLibFlexLogExtensions" as="xs:string" select="'file:/C:/Users/john.smail/workspace/ConfigCatalog/INT093_SEMA4_Out_SSK_Studio/ws/WSAR-INF/xslt/ssk147/SSK147_FunctionLib_Extensions.xsl'" static="yes"/> -->
	    		...
	    		<xsl:try>
		    		...
		    		<xsl:catch>
					    ...
					    <!-- function arguments explanation:
					     ...
					     -->
				        <xsl:value-of select="ssk:flexLog('log2', 'warn', ($err:description, xs:string($err:code), wd:Worker/wd:ID[@wd:type = 'Employee_ID'], xs:string(static-base-uri())))"/>
		    		</xsl:catch>
	    		</xsl:try>
	   			</wd:DayAfterDOB>
			</wd:Report_Entry>
    ...
    -->
    <xsl:include _href="{$sskXsltLibFlexLogExtensions}"/>
